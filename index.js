...
bot.on('messageReactionAdd', async (reaction, user) => {
	...
	if(reaction.message.id === '738444446768824372'){
		if(reaction.emoji.name == '🔴' || reaction.emoji.name == '🔵'){
			reaction.message.channel.cache.messages.fetch('738444446768824372').map(r => r).then(message => {
				reaction.message.cache.reactions.forEach(reaction => reaction.remove(reaction.message.guild.members.cache.find(member => member.id === user.id)));
			});
